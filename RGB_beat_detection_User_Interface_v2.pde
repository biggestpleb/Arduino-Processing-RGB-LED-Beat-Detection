/**
 *   Arduino RGB controller & GUI by biggest pleb
 *   
 *   Disclaimer: this was the first Java/Processing program I ever wrote, having
 *   only knowledge in basic C, and zero prior experience with any object oriented language.
 *   The GUI code is a horrible giant block of variables and redundant code sections.
 ...
 *   code (see below for that). 
 *
 *   Beat detection algorithm/code by Corey H. Walsh (coreyhwalsh@gmail.com)
 *   Beat detection code is copied directly from Corey's sketch.
 *
 *  Communication with the arduino led controller is performed via USB
 *  com port. A plain text string is sent with hue, saturation and intensity
 *  values. 
 *   
 */
...
//==========================================================================
//==========     GUI/ARDUINO VARIABLES  =====================

int totalVol, silenceTimer;
boolean isSilent = true, noNoise = true;
boolean portConnected = false;   //check if serial port is connected
...
int time = 0, time2, iTimer, sTimer, hTimer, frameTimer = 0,saveTimer;
boolean beatFlag;
Serial arduino;
...
int currentSlider = 0, wheelActive = 0;
...
void setup() {
  ...
  String[] port = loadStrings("port.txt");
  println(ports);
  for(int i=0; i<ports.length; i++){
    if( ports[i].equals(port[0]) ){
      portConnected = true; 
    }
  }
  
  if(portConnected) arduino = new Serial(this, port[0], 57600);
  time = millis();
...
void draw() {
  ...
  if( (width != prevWidth)||(height != prevHeight) ){
    rect(0, 0, width, height);
      ...
      //=========  DRAW COLOUR WHEEL  ===
      //to keep framerate up this is only drawn if the window is resized.
  ...
  stroke(0,0,0);  
  //stroke(hTemp+hOffsetTemp, sTemp+sOffsetTemp, iTemp+iOffsetTemp );  //<================== temp background color 
  noFill();
  ellipse(wheelX, wheelY, wheelDia*5/4, wheelDia*5/4);
  noStroke();
  fill(0,0,0);
  //fill(hTemp+hOffsetTemp, sTemp+sOffsetTemp, iTemp+iOffsetTemp );        // <================ Temp background color
  ...
  float barScale = barMultiplier*float(height)/50;
  ...
  colorMode(HSB, 360, 1, 1);
  if(beatDetect){
    stroke(fixHue( (hTemp+hOffset) + 0 ), 1 , 1 - (iTemp+iOffsetTemp));
    fill(hTemp+hOffsetTemp, sTemp+sOffsetTemp, iTemp+iOffsetTemp);
  }
  totalVol = 0; 
  ...
  //println(isSilent);
  //set colours of FFT bars
  if(beatDetect){
    fill(fixHue( (hTemp+hOffset) + 0), sTemp+sOffsetTemp, (iTemp+iOffsetTemp)); 
    //fill(fixHue( (hTemp+hOffset) + 0), sTemp+sOffsetTemp, iOffsetTemp);
  } else {
    fill(0,0, 0.35);
  }
  //fill(110);
 ...
 //==============================  SATURATION DECAY SLIDER  =================================
  ...
  fill(fixHue(hTemp+hOffsetTemp), sTemp, iTemp);
  triangle( triX1, triY1, triX2, triY2, triX3, triY3);
  textSize(wheelDia/4.25);
  float tempAngle = (hTemp + hOffsetTemp)%360;
  ...
  if( millis() < saveTimer ){  
    ...
    if( saveMode == 1 ){
      text("SETTINGS SAVED", width/12, numberSize*0.8); 
    } else if (saveMode == 2 ){
      text("SETTINGS LOADED", width/12, numberSize*0.8); 
    }
    textAlign(CENTER,CENTER);
  } else if( !portConnected ){
    fill(0,1,1);
    textAlign(LEFT,CENTER);
    text("WARNING: Serial Port Not Found", width/12, numberSize*0.8); 
    textAlign(CENTER,CENTER);
  }
  stroke(0,0,0);
  fill(0,0,0);
...
void stop()                                //Closes everything on stop
{
  ...
  //String[] saveData2 = split(something, ' ');
  //println(saveData);
  //saveStrings( "settings2.txt", saveData2);
